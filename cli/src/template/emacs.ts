...
function themeName(variant: AnnotatedVariant): string {
  return variant.title.kebab;
}

function fileName(variant: AnnotatedVariant): string {
  return `${themeName(variant)}-theme.el`;
}
...
const template: Template = {
  ...
  render: async function* (colorSet) {
    ...
    for (const variant of variants) {
      const { colors, isDark } = variant;
      yield {
        path: fileName(variant),
        content: Buffer.from(
          source`
            ;;; ${fileName(variant)}
            ...
            (provide-theme '${themeName(variant)})
            
            ;; Local Variables:
            ;; no-byte-compile: t
            ;; End:
            
            ;;; ${fileName(variant)} ends here
          `,
          'utf8',
        ),
      };
  ...
  renderInstructions: (paths, colorSet) => source`
    ...
    \`\`\`
    ${paths.map((p) => `cp '${p}' ~/.emacs.d/`)}
    \`\`\`

    Then load the desired theme in \`init.el\`:

    \`\`\`
    ${colorSetToVariants(colorSet)
      .map((variant) => `(load-theme '${themeName(variant)} t)`)
      .join('\n;; or ')}
    \`\`\`
  `,
};
