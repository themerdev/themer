...
2. Add the contents of ${paths.map(p => `'${p}'`).join(' and ')} to the \`Keypirinha.ini\` file.
3. Set/Append the desired theme name (${pathsToThemeNames(paths).map(name => `"${name}"`).join(' or ')}) to the \`theme\` property under the \`[gui]\` section of the \`Keypirinha.ini\` file.
`;
