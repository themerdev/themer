...
describe('themer color set inline preview', () => {
  it('should use the color set name as the default name', done => {
    Promise.all(render(colors, { colors: 'themer-colors-default' })).then(files => {
      expect(files.every(file => /themer-colors-default-(light|dark)-inline\.svg/.test(file.name))).toBe(true);
      done();
    });
  });
  it('should also accept custom names through options', done => {
    Promise.all(render(colors, { 'themer-preview-inline-name': 'test', colors: 'themer-colors-default' })).then(files => {
