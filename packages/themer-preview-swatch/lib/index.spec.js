...
describe('themer color set swatch preview', () => {
  it('should use the color set name as the default name', done => {
    Promise.all(render(colors, { colors: 'themer-colors-default' })).then(files => {
      expect(files.every(file => /themer-colors-default-(light|dark)-swatch\.svg/.test(file.name))).toBe(true);
      done();
    });
  });
  it('should also accept custom names through options', done => {
    Promise.all(render(colors, { 'themer-preview-swatch-name': 'test', colors: 'themer-colors-default' })).then(files => {
